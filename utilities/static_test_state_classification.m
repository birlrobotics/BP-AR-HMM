...
global  CONFUSION_MATRIX TIME_PERCENT PREPROCESSING_ON
...
for nState = 1:length(STATE)
    ...
    for nModel = 1:length(trainedModel) %  the same data for different state model
        ...
        dist_struct               = trainedModel{nModel}.BPARHMM_dist_struct(2); 
        F                         = trainedModel{nModel}.BPARHMM_F(2,:);
...
if PLOT_SAVE
    ...
    saveas(gHandle_testing, strcat('STATE_CLASSIFICATION','_',FOLDERS_NAME{1}),'jpg');
end
end