...
global METHOD STATE DATASET PLOT_SAVE
global TRAIN_TIMES  INERATIVE_TIMES  AUTOREGRESSIVE_ORDER  PREPROCESSING_ON     %parameters
...
if PREPROCESSING_ON
    ...
    [~, meanSigma] = data_preprocessing(cellData);
end
...
for i=1: length(cellData)
    ...
    %data_struct(i).true_labels = 1 : length(data_struct(i).obs); % initial
end
...
K = inv(diag([0.1*ones(1,m)]));  % matrix normal hyperparameter (affects covariance of matrix)
...
% Gamma(1,1)prior on the alpha and gamma
% affects \pi_z
a_alpha = 1;     %default: 1
b_alpha = 1;     %default: 1
...
settings.ploton     = PLOT_SAVE;  % Whether or not to plot the mode sequences and feature matrix while running sampler
settings.plotEvery  = 100;  %default:20  How frequently plots are displayed
